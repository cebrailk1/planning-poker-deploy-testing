...
<template>
  ...
  >
    ...
    <ul v-for="story in storyList">
      <li
        ...
        class="cursor-pointer m-1 p-1"
        :class="{
          'text-blue-300': story.name === this.$socketConnect.stagedStory.name,
        }"
      >
        {{ story.name }} : {{ story.points }} points ✅
      </li>
      <li
        v-else
        @click="stageStory(story)"
  ...
  <div
    v-else
    class="border-2 border-solid p-2 w-auto rounded-xl shadow-lg bg-opacity-80 bg-green-900 border-yellow-200"
  >
    <strong>Backlog</strong>
    <ul v-for="story in storyList">
      <li
